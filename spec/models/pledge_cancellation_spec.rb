...
describe PledgeCancellation do
  ...
  context 'validate date' do
    ...
    it 'should not be valid when date is older to last' do
      expect(subject).not_to allow_value(Date.new(2011, 1, 1)).for(:date).with_message("não pode ser menor que a data da última anulação (01/03/2012)")
    end

    it 'should not be valid when date is older then emission_date' do
      subject.stub(:emission_date).and_return(Date.new(2012, 3, 29))
      expect(subject).not_to allow_value(Date.new(2012, 3, 1)).for(:date).with_message("deve ser igual ou maior que a data de emissão")
    end
  end
end
