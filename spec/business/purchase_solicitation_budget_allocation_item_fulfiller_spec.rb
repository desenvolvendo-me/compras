...
describe PurchaseSolicitationBudgetAllocationItemFulfiller do
  ...
  context 'with purchase_solicitation_budget_allocation_items' do
    ...
    context "without process" do
      subject do
        PurchaseSolicitationBudgetAllocationItemFulfiller.new(purchase_solicitation_item_group)
      end
      ...
      it 'should clear the fulfiller when process is nil' do
        purchase_solicitation_item_group.stub(:present?).and_return(true)
        purchase_solicitation_item_group.should_receive(:fulfill_items).with(nil)
