...
describe ItbiCalculator do
  ...
  let(:setting_storage) { double("Setting") }

  subject { described_class.new(property_transfer_object, setting_storage) }
  ...
  it "should calculate the tax without amount financed" do
    ...
    setting_storage.should_receive(:fetch).with(:rate_property_transfer).and_return("2")
    setting_storage.should_receive(:fetch).with(:rate_property_transfer_funded).and_return("0.5")
