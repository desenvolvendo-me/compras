# encoding: utf-8
require 'decorator_helper'
require 'app/decorators/licitation_object_decorator'

describe LicitationObjectDecorator do
  context '#purchase_licitation_exemption_with_precision' do
    context 'when do not have purchase_licitation_exemption' do
      before do
        component.stub(:purchase_licitation_exemption).and_return(nil)
      end

      it 'should be nil' do
        expect(subject.purchase_licitation_exemption_with_precision).to be_nil
      end
    end

    context 'when have purchase_licitation_exemption' do
      before do
        component.stub(:purchase_licitation_exemption).and_return(500.0)
      end

      it 'should applies precision' do
        expect(subject.purchase_licitation_exemption_with_precision).to eq '500,00'
      end
    end
  end

  context '#build_licitation_exemption_with_precision' do
    context 'when do not have build_licitation_exemption' do
      before do
        component.stub(:build_licitation_exemption).and_return(nil)
      end

      it 'should be nil' do
        expect(subject.build_licitation_exemption_with_precision).to be_nil
      end
    end

    context 'when have build_licitation_exemption' do
      before do
        component.stub(:build_licitation_exemption).and_return(300.0)
      end

      it 'should applies precision' do
        expect(subject.build_licitation_exemption_with_precision).to eq '300,00'
      end
    end
  end
end
