class PurchaseProcessCreditorProposalBuilder
  def self.item_proposals(proposals, creditor)
    build_or_find(proposals, { creditor_id: creditor.to_i })
  end

  def self.lot_proposals(purchase_process, creditor)
    attrs = { licitation_process_id: purchase_process.id, creditor_id: creditor.to_i }
  ...
  def self.global_proposals(purchase_process, creditor)
    attrs = { licitation_process_id: purchase_process.id, creditor_id: creditor.to_i,
