module ContractsHelper
  ...
  def material_vl_unitary licitation_process, material
    LicitationProcessRatificationItem.by_licitation_process_and_material(licitation_process.id, material.id)&.last&.price || 0
  end

  def qtd_requested p_solicitation_id, material
    qtd = PurchaseSolicitationItem.joins { purchase_solicitation }
              .where { purchase_solicitation.id.eq(p_solicitation_id) }
              .where { material_id.eq(material.id) }&.last&.quantity
